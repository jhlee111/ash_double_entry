...
- Existing `Transfer` API and behavior unchanged. Within the (unreleased) multi-leg feature, `Transaction.post` changed in four observable ways:
  ...
  - A leg with a negative `amount` is refused at `[:entries, index]`. The sign belongs to `side`; a negative debit moves an account the way a credit does, so a caller permitted only to debit could credit by flipping a sign, and two negative legs satisfied the balance check (−10 == −10). Zero amounts are still allowed. (#6)
