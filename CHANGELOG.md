...
### Changed
- Internal calls build their options with `Ash.Scope.to_opts/2`; `Ash.Context.to_opts/2` is deprecated in Ash 3.x and, in the versions this library supports, is a plain alias of it — no behaviour change. (#7)
- `Transfer` now ripples a backdated (or same-millisecond) transfer through later balance rows with `:shift_balances_after` — the single-account shift entries use — once per account, instead of its own `:adjust_balance`. With an `entry_resource` configured an account's later rows come in two kinds, and `:adjust_balance`'s filter matched only `transfer_id`-keyed ones, so such a Transfer left the account's latest balance short by the whole transfer. One ripple, one ordering rule. `:shift_balances_after` is now always generated (without an `entry_resource` its filter compares only `transfer_id`); `:adjust_balance` is no longer generated by the transformer or emitted by the installer, and `AshDoubleEntry.Balance.Changes.AdjustBalance` remains only for resources that already define that action themselves.
