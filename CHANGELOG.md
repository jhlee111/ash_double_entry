...
- Existing `Transfer` API and behavior unchanged. Within the (unreleased) multi-leg feature, `Transaction.post` changed in four observable ways:
  ...
  - `skip_balance_updates` now reaches the cascaded Entries. Ash hands a managed child only the `:shared` slice of its parent's context, so a flag set the ordinary way never arrived and balance rows were written anyway, silently. (#8)
