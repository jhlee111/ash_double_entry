...
### Changed
- `Transfer`'s balance ripple (`:adjust_balance`) now also repairs entry-keyed Balance rows. With an `entry_resource` configured, an account's later balance rows come in two kinds, and the ripple's filter matched only `transfer_id`-keyed ones — so a Transfer ordered before an existing entry (backdated, or merely sharing its millisecond with a lower ULID) left the account's latest balance short by the whole transfer. `:shift_balances_after` had always compared both columns; the two ripples now agree. Resources without an `entry_resource` are unaffected.
- Existing `Transfer` API and behavior unchanged. Within the (unreleased) multi-leg feature, `Transaction.post` changed in four observable ways:
  ...
  - A leg with a negative `amount` is refused at `[:entries, index]`. The sign belongs to `side`; a negative debit moves an account the way a credit does, so a caller permitted only to debit could credit by flipping a sign, and two negative legs satisfied the balance check (−10 == −10). Zero amounts are still allowed. Per-leg faults — a negative amount, a misspelled key — are all reported together and before the journal-level checks, so a negative leg is named even when the journal is also unbalanced. (#6)
